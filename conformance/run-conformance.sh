    ...
    "containers": [
      {
        ...
        "command": ["./run_test.sh"],
        "args": ["--junit-xml=/output/calrissian-conformance.xml", "--classname=calrissian", "RUNNER=calrissian", "-n2", "--verbose", "EXTRA=--max-ram 8G --max-cores 4 --default-container debian:stretch-slim"],
        "stdin": true,
        "stdinOnce": true,
        "tty": true,
        "env": [
